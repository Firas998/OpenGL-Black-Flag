...
vec3 interpolation_direction(float t, buffer<vec3> const& key_positions, buffer<float> const& key_times) {
    ...
    vec3 d0 = 2 * 0.5 * (p2 - p0) / (t2 - t0);
    vec3 d1 = 2 * 0.5 * (p3 - p1) / (t3 - t1);

    mat4 points = { p0.x, p0.y, p0.z, 0,
                    d0.x, d0.y, d0.z, 0,
                    p1.x, p1.y, p1.z, 0,
                    d1.x, d1.y, d1.z, 0 };

    vec4 u = { 3*(float)pow(alpha, 2), 2*(float)alpha, 1, 0 };

    vec4 coeffs = spline * u;
    vec4 ci = transpose(points) * coeffs;

    vec3 const p = { ci.x,ci.y,ci.z };
...
vec3 cardinal_spline_interpolation(float t, float t0, float t1, float t2, float t3, vec3 const& p0, vec3 const& p1, vec3 const& p2, vec3 const& p3, float K)
{
    ...
    vec3 d0 = 2 * K * (p2 - p0) / (t2 - t0);
    vec3 d1 = 2 * K * (p3 - p1) / (t3 - t1);
    ...
    vec4 u = { (float) pow(alpha, 3), (float) pow(alpha, 2), alpha, 1 };
